...
| **TUI Control Panel** | Terminal-based setup wizard and real-time monitoring dashboard |
...
The Terminal User Interface (TUI) provides a unified control panel for all sensor operations:
...
| Panel | Capabilities |
|-------|-------------|
| **Quick Setup** | One-click Demo/Live bundles, token generation, `.env` auto-write |
| **Profiles & Presets** | Save/load named configurations (SOC Tactical, PCAP Forensics, etc.) |
| **Configuration** | Sensor ID, interface selection (dropdown), PCAP path, geo-location |
| **Interface Automation** | USB/WiFi detection, monitor/managed mode switching |
| **Backend & Lab** | Stack health check, dependency install, lab up/down/reset |
| **Security Audit** | Run compliance checklists (Home/SME profiles) with results table |
| **Diagnostics** | Stream pytest output with unit/integration test selection |
| **Operations** | Background daemon management, production Docker deployment |
| **Live Dashboard** | 4-panel real-time view: system health, network feed, alerts, log stream |

### Workflow

1. **Setup Screen**: Configure mode (Live/Mock/PCAP), select interface, set controller URL
2. Press **F5** or click **START SENSOR** to switch to the live dashboard
3. **Dashboard**: Monitor networks, alerts, and system health in real-time
...
```bash
# Uses ops/.env.lab for lab-specific secrets (auto-generated by lab-up)
docker compose --env-file ops/.env.lab -f ops/docker-compose.lab.yml up -d --build
```

> [!NOTE]
> If `ops/.env.lab` does not exist yet, run `make lab-up` first — it generates the lab
> secrets, runtime tokens, and starts the stack in one step.
...
```bash
# 1. Create production secrets from the example template
cp ops/.env.prod.example ops/.env
nano ops/.env   # ← fill in real secrets
...
# SME profile with mock data
python sensor/audit.py --sensor-id test --mock --profile sme --output report.json
```
...
*   **[TUI Guide](docs/lab/tui_guide.md)**: Terminal Control Panel usage.
...
* **Windows/macOS Sensor:** The sensor capture driver requires Linux with monitor-mode capable WiFi adapters. On Windows/macOS, use Mock mode or PCAP replay for testing. Controller and Dashboard work on all platforms.
