...
def proxy_app():
    ...
    app.wsgi_app = TrustedProxyMiddleware(
        app.wsgi_app, trusted_cidrs=["127.0.0.1", "10.0.0.0/8"], x_for=1, x_proto=1
    )
...
def test_direct_untrusted_spoofing(proxy_app):
    ...
    headers = {"X-Forwarded-Proto": "https", "X-Forwarded-For": "5.6.7.8"}
...
def test_trusted_proxy_double_spoof(proxy_app):
    ...
    headers = {"X-Forwarded-Proto": "https", "X-Forwarded-For": "192.168.1.5, 1.2.3.4"}
