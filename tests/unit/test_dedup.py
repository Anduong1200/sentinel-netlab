...
from controller.dedup.store import EventStore, EventState
...
def test_suppression():
    ...
    state = EventState(
        first_seen=now, last_seen=now, last_emitted=now, 
        count=1, max_severity=Severity.LOW, max_risk_score=10.0
    )
