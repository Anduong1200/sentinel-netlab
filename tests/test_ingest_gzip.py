...
with (
    patch("controller.api.deps.db.create_all"),
...
class TestIngestGzip(unittest.TestCase):
    ...
    def test_gzip_hmac_ingest(self, mock_enqueue, mock_token_query):
        ...
        mock_enqueue.return_value = ("ack-123", False)
        ...
        h = hmac.new(
            os.environ["CONTROLLER_HMAC_SECRET"].encode(), digestmod=hashlib.sha256
        )
        ...
        # Send Request
        response = self.client.post(path, data=payload_bytes, headers=headers)
