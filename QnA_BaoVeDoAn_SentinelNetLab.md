...
**Trả lời:** Controller được thiết kế theo hướng bất đồng bộ (Asynchronous processing). Các payload từ Sensor gửi đến REST API sẽ được tiếp nhận nhanh và ném thẳng vào Message Broker/Queue (như Redis). Các Background Workers (như Celery) sẽ âm thầm lấy dữ liệu từ hàng đợi ra để phân tích, ghi vào Database. Nhờ vậy, API Server không bao giờ bị block/nghẽn khi xử lý I/O nặng.
...
4. **Cấp độ Data:** Sử dụng Persistent Queue (SQLite Spool) lưu tạm dữ liệu lúc rớt mạng, tự động phục hồi "inflight packets" và đẩy lại (drain backlog) khi mạng có lại.
