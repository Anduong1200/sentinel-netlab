...
class NTPSyncManager:
    ...
    def get_sync_status(self) -> dict:
        ...
        return {
            ...
            "max_drift_ms": max(self.sensor_offsets.values(), default=0)
            - min(self.sensor_offsets.values(), default=0),
        }
...
class TrilaterationSolver:
    ...
    def solve(
        self, sensors: list[SensorPosition], distances: list[float]
...
class HeatmapGenerator:
    ...
    def __init__(
        self, width_m: float = 50.0, height_m: float = 50.0, resolution: float = 0.5
    ):
        """
    ...
    def export_png(
        ...
        im = ax.imshow(
            self.grid, cmap=cmap, extent=extent, origin="lower", vmin=-100, vmax=-30
        )
        ...
        if targets:
            for t in targets:
                ...
                ax.annotate(
                    t.bssid[-8:] if t.bssid else "Target",
                    (t.x, t.y),
                    color="white",
                    fontsize=8,
                )
...
class GeoMapper:
    ...
    def process_samples(self, samples: list[RSSISample]) -> PositionEstimate | None:
        """
...
def main():
    ...
    for i in range(5):  # 5 measurements
        ...
        if estimate:
            error = math.sqrt(
                (estimate.x - target_x) ** 2 + (estimate.y - target_y) ** 2
            )
            print(
