...
class SqliteQueue:
    ...
    def push(self, payload: dict[str, Any], batch_id: str) -> bool:
        ...
        with self._lock:
            ...
            # Check size limits
            cursor = conn.execute("SELECT COUNT(*), SUM(LENGTH(payload_json)) FROM spool WHERE state != 'acked'")
            row = cursor.fetchone()
            count = row[0] or 0
    ...
    def nack(self, batch_id: str, error: str) -> None:
        ...
        with self._lock:
            ...
            conn.commit()
            logger.warning(f"NACK: {batch_id} retry #{attempts} in {backoff:.1f}s. Error: {error}")
