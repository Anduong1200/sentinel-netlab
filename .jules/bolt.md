...
## 2026-04-18 - Performance optimization for high-frequency sliding window cleanup
**Learning:** In high-frequency ingestion algorithms (like packet analysis or sliding windows), bounded lists that rely on `pop(0)` in loops (e.g., `while lst and lst[0] < cutoff: lst.pop(0)`) suffer from O(N) memory allocation overhead on every iteration, leading to O(K*N) performance for K removals, creating significant bottlenecks during event floods.
**Action:** Replace `while ... pop(0)` loops with a combination of `bisect.bisect_left` and in-place slice deletion (`del lst[:idx]`). This achieves O(log N + N) batch removal, avoiding continuous memory reallocation overhead and dramatically improving performance during high-volume event processing.
