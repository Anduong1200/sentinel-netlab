...
class RecommendationEngine:
    ...
    def analyze(network_data: dict[str, Any], risk_result: dict[str, Any]) -> list[str]:
        ...
        if network_data.get('is_evil_twin'):
             recs.append("CRITICAL: Detect Evil Twin attack. Disconnect immediately and verify AP MAC address.")

        # 2. Encryption Issues
        enc = network_data.get('encryption', 'Unknown').upper()
        if "OPEN" in enc or features.get('enc_score', 0) > 0.8:
            recs.append("Critical: Open network detected. Use VPN for all traffic. Do not access sensitive banking/email.")
        elif "WEP" in enc:
             recs.append("High Risk: WEP is broken. Upgrade router to WPA2/WPA3 immediately.")
        ...
        if network_data.get('wps_enabled'):
            recs.append("hardening: Disable WPS (WiFi Protected Setup) to prevent PIN brute-force.")

        if "Default" in network_data.get('ssid', '') or features.get('ssid_suspicious', 0) > 0.5:
             recs.append("hardening: Change default SSID (network name) to reduce targeting profile.")

        # 4. Signal/Location
        if features.get('rssi_norm', 0) > 0.9 and risk_result.get('risk_score', 0) > 70:
            recs.append("Physical Security: High-risk AP is in close proximity. Check for rogue devices nearby.")
