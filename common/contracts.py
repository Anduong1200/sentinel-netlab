...
class RawFrame(BaseModel):
    ...
    mac_dst: Optional[str] = Field(None, pattern=r"^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$")
    ...
    # Sequence
    sequence_number: Optional[int] = Field(None, ge=0, le=4095)
    fragment_number: Optional[int] = Field(None, ge=0, le=15)
    ...
    # Raw data
    frame_length: int = 0
    raw_bytes: Optional[bytes] = Field(None, exclude=True)
...
class NormalizedFrame(BaseModel):
    ...
    mac_src_hash: Optional[str] = None
    bssid_hash: Optional[str] = None
    
    # OUI enrichment
    vendor_src: Optional[str] = None
    vendor_bssid: Optional[str] = None
...
class SensorStatus(BaseModel):
    ...
    channel_current: Optional[int] = None
